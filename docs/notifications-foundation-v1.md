...
**Статус:** завершено 25.09.2026; усі 28 тестів пройдено без пропусків у GitHub Actions і локально користувачем.
...
### Результат CI — 25.09.2026

[Backend checks, запуск 36140650684](https://github.com/Mahone1008/STechbaza-iot/actions/runs/36140650684)
на commit `07298f13692c4b5b3f730377ebd107869eb157a3`: **Ran 28 tests — OK**,
без пропусків. Підтверджено всі вісім нових сценаріїв, зокрема MQTT →
Alarm → notification → HTTP acknowledge → recovery. Застосування міграцій,
відкат 0015 → 0014 і повторне оновлення до 0015 успішні у тимчасовій CI-базі.

Перший запуск 36140447791 також пройшов усі 28 тестів. Виявлене попередження
SQLAlchemy про застарілий `Result.tuples()` прибрано; повторний запуск успішний.
Це автоматична перевірка програмного стенда. Локальний результат користувача
зафіксовано в розділі 9; фізичний ESP32/VFD перевіряється окремо.
...
Результат виконання користувачем зафіксовано нижче.

## 9. Локальне підтвердження — 25.09.2026

Користувач надав чотири скриншоти виконання PowerShell-блоку на Windows
із Docker Compose. Перевірено commit `2b50c9b1359fcc1c39c7cf23d79855800daf85d1`.

| Перевірка | Підтверджений результат |
|---|---|
| Оновлення репозиторію | Fast-forward `ba60a03 → 2b50c9b` із `Mahone1008/STechbaza-iot` |
| Збірка backend | Успішна |
| Міграція | Виконано upgrade `20260925_0014 → 20260925_0015`; блок продовжився без помилки |
| Повний набір PostgreSQL/MQTT/JWT тестів | `Ran 28 tests in 4.228s — OK`, без пропусків |
| HTTP acknowledge: JWT та права | `PASS`: unauthenticated/revoked 401, tenant 404, viewer 403, resolved 409 |
| HTTP acknowledge: повтор і аудит | `PASS`: один transition, збережено actor/session/role |
| Прибирання даних HTTP acknowledge | `PASS`: тестова транзакція відкотилася, Device/Alarm/User не залишилися |
| Повторний запуск та health | `ok / techbaza-backend / 0.31.0` |
| Завершення всього блоку | Фінальний зелений `PASS` |

Рядок результату `alembic current` обрізано на скриншоті. Окремо як видимий
`0015 (head)` його не зараховано; видно сам upgrade, успішне продовження блоку
та виконання всіх інтеграційних тестів.

Traceback `Injected temporary database processing failure` є навмисною
імітацією тимчасового збою в тесті повторної доставки MQTT. Відповідний тест
завершився `ok`, а весь набір — `OK`.

Операція 7 — завершено. Етап 7 — завершено в межах Events & Alarms Core
та in-app notifications foundation. Зовнішні канали, UI, навантаження,
retention і фізичне обладнання залишаються окремими наступними роботами,
зазначеними в розділі 6.
